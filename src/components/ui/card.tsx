...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      "rounded-xl bg-card-custom text-card-foreground flex flex-col h-full",
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <a
    ...
    href="#"
    className={cn(
      "text-sm text-muted-foreground hover:text-foreground transition-colors inline-flex items-center gap-1 py-10",
      className
    )}
    {...props}
  >
